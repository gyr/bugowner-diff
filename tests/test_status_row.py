...
def test_status_members_are_strings() -> None:
    assert isinstance(Status.ADDED, str)
...
def test_status_member_formats_as_its_bare_csv_cell(member: Status) -> None:
    # A plain Enum would render as "Status.ADDED" here, which is the defect this
...
def test_status_row_fields_are_ordered_like_the_csv_columns() -> None:
...
def test_status_row_holds_the_tagged_owner_sets_of_both_sides() -> None:
    ...
    assert row.status is Status.CHANGED


def test_status_row_is_frozen() -> None:
    row = StatusRow("spice", frozenset({"group:team-a"}), frozenset(), Status.REMOVED)
...
def test_rows_compare_by_exact_set_equality_not_by_owner_ordering() -> None:
    ...
    first = StatusRow(
        "crmsh", frozenset({"user-a"}), frozenset({"user-a", "user-b"}), Status.CHANGED
    )
    second = StatusRow(
        "crmsh", frozenset({"user-a"}), frozenset({"user-b", "user-a"}), Status.CHANGED
    )
